...
struct MaterialSolid : public AMaterial {
	...
	constexpr MaterialSolid(Color ambiant, Color diffuse, Color specular, float shininess, float opacity = 1) noexcept
	...
	auto getShader() const noexcept -> const ShaderProgram & { return getMaterialShader<MaterialSolid>(); }
